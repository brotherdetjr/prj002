...
static void draw_shake_prompt(int sec_left)
{
    serial_log("Screen: shake prompt (%d)\n", sec_left);
    screen->fillScreen(BLACK);
    slog_y = X_TIME;
    slog("Shake to enter");
    slog("config mode");
    char buf[32];
    snprintf(buf, sizeof(buf), "within %d seconds", sec_left);
    slog(buf);
    screen_flush();
}
...
static const char CONFIG_HTML[] =
    ...
    ".pw-wrap input{padding-right:3.5em;-webkit-text-security:disc}"
    ".pw-wrap input.show{-webkit-text-security:none}"
    ".pw-toggle{position:absolute;right:.4em;top:50%;transform:translateY(-50%);"
    "background:none;border:1px solid #aaa;border-radius:3px;padding:.1em .4em;"
    "font-size:.85em;color:#444;cursor:pointer;margin:0;width:auto}"
    "</style></head><body>"
    ...
    "<button type=button class=pw-toggle onclick=\"var i=document.getElementById('pass');"
    "i.classList.toggle('show');this.textContent=i.classList.contains('show')?'Hide':'Show'\">"
    "Show</button>"
    "</div>"
...
void setup()
{
    ...
    // AP must start before the display — WiFi RF calibration briefly disables the
    // flash cache; LCD DMA firing during that window causes a Cache panic.
    // Starting AP unconditionally here handles calibration and makes DepartBoard
    // available for the optional-config portal from the very first boot.
    ...
    // Bind the web server socket — after lwIP stack exists, before any portal_start().
    portal_server_init();

    // Display init (RF calibration is complete — AP is up).
    ...
    // 5-second startup shake window.  IMU needs to stabilise after AP bring-up;
    // the wait is usually short because softAP() itself takes ~1 s.
    while (millis() < imu_ready_at_ms) delay(10);
    {
        unsigned long shake_end = millis() + 5000;
        bool user_shook = false;
        int sec_shown = -1;
        while (millis() < shake_end) {
            int sec_left = (int)((shake_end - millis() + 999) / 1000);
            if (sec_left != sec_shown) {
                sec_shown = sec_left;
                draw_shake_prompt(sec_left);
            }
            if (check_shake()) { user_shook = true; break; }
            delay(10);
        }
        if (user_shook) {
    ...
    // BaU path: attempt STA connection now that the shake window has closed.
    bool wifi_ok = false;
    char reason[64] = "";
...
void loop()
{
    ...
    check_shake();  // keeps IMU debug state updated
    ...
    case OPTIONAL_CONFIG:
        // Portal stays active until user submits the form, which triggers ESP.restart().
        break;
